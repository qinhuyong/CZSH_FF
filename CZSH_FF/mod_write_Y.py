...
def get_lammps_input_cementff(name, entries_crystal, entries_bonds, entries_angle, supercell, zinc_summary=None):
    ...
    with open(name, "w") as f:
        ...
        for entry in entries_crystal:
            ...
            f.write(fmt.format(int(entry[0]), 1, lammps_type, float(entry[2]), *entry[3:], label))
...
def write_output( isample, entries_crystal, entries_bonds, entries_angle, size, crystal_rs, water_in_crystal_rs,
    ...
    if write_lammps_cementff:
        ...
        name = name +".data"
        name = os.path.join(path, name)
        get_lammps_input_cementff(name, entries_crystal, entries_bonds, entries_angle, supercell, zinc_summary)
